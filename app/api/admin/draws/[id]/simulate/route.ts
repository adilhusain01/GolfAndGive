...
import { randomDraw, algorithmicDraw, countMatches, getPrizeTier } from "@/lib/draw-engine";
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (draw.logic === "algorithmic") {
    const { data: entries } = await adminSupabase
      ...
      .in(
        "user_id",
        (await adminSupabase.from("subscriptions").select("user_id").eq("status", "active")).data?.map((s: any) => s.user_id) ?? []
      );
