  ...
  return (
    <div className="max-w-3xl space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">All monthly draws you've participated in.</p>
      </div>
      ...
      {(!entries || entries.length === 0) ? (
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            ...
            <p>No draw entries yet. Add scores to participate in the next draw.</p>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={entry.id}>
              <CardContent className="py-4">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-semibold">{entry.draws?.draw_month && getMonthLabel(entry.draws.draw_month)}</p>
                    <Badge variant={entry.draws?.status === "published" ? "default" : "secondary"} className="text-xs mt-1">
                      {entry.draws?.status}
                    </Badge>
                  </div>
                  {entry.prize_tier && (
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {entry.draws?.winning_numbers?.length > 0 && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
                      ...
                      <div className="flex gap-1">
                        {entry.draws.winning_numbers.map((n: number, i: number) => (
                          <span key={i} className="size-8 rounded-full bg-amber-500/10 border-2 border-amber-500/40 text-amber-500 text-xs font-bold flex items-center justify-center">
                            {n}
                          </span>
                        ))}
                      </div>
                ...
                {entry.match_count !== null && entry.draws?.status === "published" && (
                  <p className="text-xs text-muted-foreground mt-2">
                    Matched <strong>{entry.match_count}</strong> number{entry.match_count !== 1 ? "s" : ""}
                    {entry.match_count >= 3 ? " 🎉" : ""}
                  </p>
                )}
              </CardContent>
